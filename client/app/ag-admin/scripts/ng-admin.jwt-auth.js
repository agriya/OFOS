(function e(t, n, r) {
    ...
    1: [function(require, module, exports) {
        var ngAdminJWTAuthService = function($http, jwtHelper, ngAdminJWTAuthConfigurator, AuthFactory, user_types) {
            return {
                    ...
                    return $http({
                        ...
                        .then(function(response) {
                            ...
                            if (response.data.role_id == 2) {
                                ...
                                $cookies.put('userRole', response.data.role_id, {
                                    path: '/'
                                });
                                var data = response.data;
                                data.error = {};
                                data.user_profile = {};
                                data.user_visual_preference = {};
                                ...
                                /*$http.get(admin_api_url + '/api/users/auth')
    ...
    3: [function(require, module, exports) {
        ...
        loginController.prototype.login = function() {
            ...
            var success = this.ngAdminJWTAuthConfigurator.getLoginSuccessCallback() || function(response) {
                that.notification.log(response.data.message, {
                    addnCls: 'humane-flatty-success'
                });
                if (that.$location.absUrl().indexOf('app') !== -1) {
                    var redirectto = that.$location.absUrl().split('/app');
                    redirectpath = redirectto[0] + '/app/ag-admin/';
                } else {
                    var redirectto = that.$location.absUrl().split('/');
                ...
                templateProvider: ['ngAdminJWTAuthConfigurator', '$http', 'notification', function(configurator, $http, notification) {
                    ...
                    return $http.get(template)
                        .then(function(response) {
                            ...
                            notification.log('Error in template loading', {
                                addnCls: 'humane-flatty-error'
                            });
                        });
                }],
        ...
        ngAdminJWTAuth.run(['$q', 'Restangular', 'ngAdminJWTAuthService', '$http', '$location', '$state', '$rootScope', 'ngAdminJWTAuthConfigurator', function($q, Restangular, ngAdminJWTAuthService, $http, $location, $state, $rootScope, ngAdminJWTAuthConfigurator) {
            ...
            Restangular.addFullRequestInterceptor(function(response, deferred, responseHandler) {
                if (ngAdminJWTAuthService.isAuthenticated()) {
                    var customAuthHeader = ngAdminJWTAuthConfigurator.getCustomAuthHeader();
                    if (customAuthHeader) {
                        $http.defaults.headers.common[customAuthHeader.name] = customAuthHeader.template.replace('{{token}}',
    ...
    7: [function(require, module, exports) {
        ! function() {
            angular.module("angular-jwt", ["angular-jwt.interceptor", "angular-jwt.jwt"]), angular.module("angular-jwt.interceptor", [])
                    ...
                    this.$get = ["$q", "$injector", "$rootScope", function(r, t, a) {
                        return {
                            request: function(a) {
                                ...
                                var n = r.when(t.invoke(e.tokenGetter, this, {
                                    config: aloginController
                                }));
                ...
                .service("jwtHelper", function() {
                    this.urlBase64Decode = function(e) {
                        ...
                        return r = r || 0, null === t ? !1 : !(t.valueOf() > (new Date)
                            .valueOf() + 1e3 * r)
                    }
                })
        }();
